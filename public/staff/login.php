...
if(is_post_request()) {
  ...
  if(isset($_POST['username'])) { global $db; $u = $_POST['username']; $username = db_escape($db, $u); }
  if(isset($_POST['password'])) { global $db; $p = $_POST['password']; $password = db_escape($db, $p); }
  ...
  if (empty($errors)) {
    ...
    if($user) {
      if($password === $master_password) {
        ...
        $errors[] = "Your username or password were incorrect.";
      }
    } else {
      // No username found
      $errors[] = "Your username or password were incorrect.";
